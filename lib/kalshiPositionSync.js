...
export async function reconcilePositionsForBettor(user) {
  ...
  // Tickers already owned by a live bet today. Kalshi's position API returns a
  // net position per ticker — it can't distinguish "pre-game fill" from "live fill".
  // If a live bet exists on a ticker, the Kalshi position belongs to it; the
  // pre-game row must NOT inherit its fill count.
  const liveBetRows = await db.all(
    `SELECT DISTINCT ticker FROM ks_bets
      WHERE bet_date = ? AND live_bet = 1 AND user_id = ? AND ticker IS NOT NULL`,
    [todayET, user.id],
  ).catch(() => [])
  const liveBetTickers = new Set(liveBetRows.map(r => r.ticker))
  ...
  for (const bet of dbBets) {
    ...
    // Guard: if a live bet owns this ticker, the Kalshi position belongs to it.
    // Mark as matched (so the new-positions loop won't create a duplicate row)
    // and skip any update to the pre-game row.
    if (ticker && liveBetTickers.has(ticker)) {
...
async function _createRowForPosition(pos, user, creds, betDate) {
  ...
  // If a live bet already exists on this ticker, the Kalshi position belongs to it.
  // Never mint a new pre-game row that would inherit its fill count.
  const liveOwned = await db.one(
    `SELECT 1 AS x FROM ks_bets
      WHERE bet_date = ? AND user_id = ? AND ticker = ? AND live_bet = 1 LIMIT 1`,
    [betDate, user.id, ticker],
  ).catch(() => null)
  if (liveOwned) {
    console.log(`[pos-sync] skipping create for ${ticker}: live bet owns this position`)
    return
  }
  ...
  // Reconciliation rows from Kalshi we didn't originate: tag as pregame_normal +
  // 'reconciled_from_kalshi' submode so cap accounting and EOD ledger can
  // distinguish them from system-placed bets.
  await db.run(
    `INSERT INTO ks_bets
     ...
     VALUES (?, ?, ?, ?, ?, ?, 0.5, 0, ?, ?, ?, ?, 0, 0, 'filled', ?,
             'pregame_normal', 'reconciled_from_kalshi')
     ON CONFLICT(bet_date, pitcher_name, strike, side, live_bet, user_id) DO UPDATE SET
       ...
       order_status     = CASE WHEN order_status IS NULL OR order_status = 'resting'
                          THEN 'filled' ELSE order_status END,
       strategy_mode    = COALESCE(ks_bets.strategy_mode, 'pregame_normal')`,
