...
// ------------------------------------------------------------------
// Sonnet: Pre-bet K-market preflight — runs ~2.5h before first pitch
//
// Receives pre-classified structured intelligence (signals already tagged
// by code-layer keyword matching + numeric computations). Claude's job is
// purely synthesis: resolve conflicts, weigh evidence, final call.
// ------------------------------------------------------------------
const SCOUT_K_MARKET_SYSTEM = `You are the final decision-maker for a Kalshi MLB pitcher strikeout market betting system.
You run 2.5 hours before first pitch. All data has already been classified by rules before reaching you.
Your job: synthesize pre-classified signals and make one final judgment.

ACTIONS:

"skip" — Do NOT place the bet. Require one of:
  - Any HIGH-confidence skip signal (pitch count limit, confirmed injury, bullpen day)
  - K prop gap of -1.0 or worse (sharp money at DK/FD disagrees by 1+ Ks — they have same-day info we don't)
  - Multiple MEDIUM-confidence skip signals converging (e.g. news concern + fresh bullpen + line drop)
  - Last start had ≥105 pitches AND days rest ≤ 4 (likely on a pitch limit today)
  - Last start had ≥ 110 pitches AND any caution news signal (definitely limited today)

"boost" — Place the bet with extra confidence. Require TWO or more:
  - Tired bullpen (starter gets longer leash)
  - Multiple starters confirmed out of opponent lineup
  - Pitcher velocity/command signal from today
  - K prop gap of +1.5 or better (sharp market agrees we're getting a steal)
  - Last start had elite K total (e.g. 8+ Ks in 6+ IP) + no negative signals today

"proceed" — Default. Use when:
  - Signals are absent, mixed, or only low-confidence
  - Single weak signal without corroboration

CRITICAL RULES:
  - Absence of news = proceed. NEVER skip on no evidence.
  - The K prop gap is your most reliable signal — DK/FD sharp books have better same-day intelligence than our model.
  - Low-confidence news signals (Google News) never alone justify a skip.
  - HIGH-confidence sources (Rotowire, MLB official) carry real weight.
  - A single MEDIUM signal is not enough to skip.
  - Short rest (≤ 4 days) is a CAUTION flag but not alone a skip unless combined with high pitch count.

Output VALID JSON only — no prose, no code fences.
Schema: {"action":"proceed|skip|boost","reason":"one sentence citing the specific signal(s) that drove this","confidence":0.0-1.0}`

export async function scoutKMarket({
  pitcherName, gameLabel, kTrend,
  skipSignals, boostSignals, neutralFlags,
  kPropGap, lineDelta, bullpenIp2d, bullpenSignal,
  weatherSummary, umpireChanged, opponentNews,
  lastStartContext,
}) {
  const fmt = arr => arr?.length ? arr.map((s, i) => `  ${i + 1}. ${s}`).join('\n') : '  (none)'

  const lastStartLine = lastStartContext
    ? `${lastStartContext.lastStartDate}: ${lastStartContext.lastStartPitches ?? '?'} pitches, ${lastStartContext.lastStartKs ?? '?'} Ks in ${lastStartContext.lastStartIP ?? '?'} IP | ${lastStartContext.daysRest} days rest`
    : 'unavailable'

  const user = `Pitcher: ${pitcherName}
Game: ${gameLabel}

── Pre-classified signals ──
SKIP signals:
${fmt(skipSignals)}
...
Cautionary flags (neutral):
${fmt(neutralFlags)}

── Numeric context ──
K prop gap (model λ minus DK line): ${kPropGap != null ? kPropGap.toFixed(1) + ' Ks' : 'unavailable'}
  (negative = DK thinks fewer Ks than our model; -1.0 or worse is meaningful)
Game total movement: ${lineDelta != null ? (lineDelta >= 0 ? '+' : '') + lineDelta.toFixed(1) + ' runs from open' : 'unavailable'}
Bullpen IP last 2 days: ${bullpenIp2d != null ? bullpenIp2d.toFixed(1) + ' IP (' + (bullpenSignal || 'normal') + ')' : 'unavailable'}
Weather: ${weatherSummary || 'unavailable'}
Umpire changed since edge calc: ${umpireChanged ? 'YES' : 'no'}
...
── Opponent lineup news ──
${opponentNews?.length ? opponentNews.map((s, i) => `  ${i + 1}. ${s}`).join('\n') : '  (no lineup news)'}
...
Return JSON only.`
  ...
  const callPromise = call({
    ...
    catch { return { action: 'proceed', reason: 'parse-error', confidence: 0 } }
  })
  const timeoutPromise = new Promise(resolve =>
    setTimeout(() => resolve({ action: 'proceed', reason: 'timeout', confidence: 0 }), TIMEOUT_MS)
  )
