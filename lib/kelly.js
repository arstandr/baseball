...
export function kellySizing(modelProb, marketPrice, side, isMaker = false) {
 ...
 */
export function correlatedKellyDivide(edges, isMaker = false) {
  if (!edges || edges.length === 0) return []
  if (edges.length === 1) {
    const { modelProb, marketPrice, side } = edges[0]
    const result = kellySizing(modelProb, marketPrice, side, isMaker)
  ...
  for (const side of ['YES', 'NO']) {
    ...
    const raw = group.map(e => {
      const k = kellySizing(e.modelProb, e.marketPrice, e.side, isMaker)
  ...
  // Convert Map back to a dense array aligned 1-to-1 with input edges.
  // Using a Map (instead of sparse array assignment) ensures every slot is
  // explicitly present — when all qualifying edges fall on one side (all YES
  // or all NO), the other side's indices would otherwise remain undefined,
  // silently dropping those bets in the caller.
