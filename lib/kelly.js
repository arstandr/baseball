...
export function kellySizing(modelProb, marketPrice, side) {
  // Edge from our perspective
  const edge = side === 'YES'
    ? modelProb - marketPrice
    : (1 - modelProb) - (1 - marketPrice)

  if (edge <= 0) return { betSize: 0, kellyFraction: 0, edge, rationale: 'no edge' }

  // Price we pay per contract
  const price = side === 'YES' ? marketPrice : (1 - marketPrice)
  if (price <= 0) return { betSize: MIN_BET, kellyFraction: 0, edge, rationale: 'price too low, min bet' }

  // Full Kelly = edge / (1 - price)  [win = 1-price, lose = price per unit]
  // This equals edge/price when side=NO since "price" = 1-marketPrice
