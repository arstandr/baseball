...
export function opportunityDiscount(remaining) {
  // Input: count of pitchers with actual edge in the current run (not raw slate size).
  // Discounting is light — the portfolio cap is the primary budget constraint.
  if (remaining >= 5) return 0.75  // was: 7→0.65 (too aggressive on typical slates)
  if (remaining >= 3) return 0.85  // was: 4→0.80
