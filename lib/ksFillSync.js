...
export async function syncFillsForBettor(user, todayET) {
  ...
  // Use ET date passed in — date('now') is UTC and breaks after midnight ET
  const today = todayET ?? new Date().toLocaleString('sv-SE', { timeZone: 'America/New_York' }).slice(0, 10)
  ...
  const bets = await db.all(
    `SELECT id, side, order_id, ticker, filled_contracts, order_status
     FROM ks_bets
  ...
  for (const bet of bets) {
    try {
      ...
      // Store price from the perspective of the side we purchased
