...
import { TEAM_NAMES, TEAM_SLUGS } from './teams.js'
...
async function getKPropGap(pitcherName, gameDate) {
  try {
    // Try exact match first (case-insensitive), then last-name LIKE with first-initial disambiguation
    let row = await db.one(
      `SELECT dk_line, pitcher_name FROM dk_k_props WHERE prop_date = ? AND lower(pitcher_name) = lower(?) LIMIT 1`,
      [gameDate, pitcherName],
    )
    if (!row) {
      const lastName     = pitcherName.split(' ').pop().toLowerCase()
      const firstInitial = pitcherName.charAt(0).toLowerCase()
      const candidates   = await db.all(
        `SELECT dk_line, pitcher_name FROM dk_k_props WHERE prop_date = ? AND lower(pitcher_name) LIKE ?`,
        [gameDate, `%${lastName}%`],
      )
      if (candidates.length === 1) {
        row = candidates[0]
      } else if (candidates.length > 1) {
        row = candidates.find(c => c.pitcher_name.charAt(0).toLowerCase() === firstInitial) ?? null
      }
    }
    if (!row) return null
    // Look up our model's average lambda for this pitcher today across all bets
    const bets = await db.all(
      `SELECT lambda, strike FROM ks_bets WHERE bet_date = ? AND lower(pitcher_name) = lower(?) AND lambda IS NOT NULL`,
      [gameDate, pitcherName],
    )
...
async function getLastStartContext(pitcherId) {
  try {
    const season = new Date().getFullYear()
    const rows = await db.all(
      `SELECT game_date, pitches, ks, ip, bf FROM pitcher_recent_starts
       WHERE pitcher_id = ? AND season = ?
       ORDER BY game_date DESC LIMIT 1`,
      [String(pitcherId), season],
    )
    if (!rows.length) return null
    const last = rows[0]
    const daysRest = Math.round((Date.now() - new Date(last.game_date + 'T12:00:00Z').getTime()) / 86_400_000)
    return {
      lastStartDate:    last.game_date,
      lastStartPitches: last.pitches ?? null,
      lastStartKs:      last.ks ?? null,
      lastStartIP:      last.ip != null ? Number(last.ip).toFixed(1) : null,
...
export async function runPreflightCheck(entry) {
  ...
  const [
    ...
    kPropData, lineDelta, bullpenData, weatherData, umpireData, lastStartCtx,
  ] = await Promise.all([
    ...
    getLastStartContext(pitcher_id),
  ])
  ...
  // K prop hard skip: if gap is -1.0 or worse (DK disagrees by 1+ K), skip without Sonnet
  if (kPropData?.gap != null && kPropData.gap <= -1.0) {
    return {
      action:     'skip',
