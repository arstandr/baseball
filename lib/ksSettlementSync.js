...
export async function syncSettlementsForUser(user) {
  ...
  // Always persist live balance — catches deposits that arrive with no new settlements
  if (bal) {
    await db.run(
  ...
  for (const s of settlements) {
    ...
    // Check for contradictions: local result conflicts with Kalshi settlement
    const settledWrong = await db.all(
      `SELECT id, side, result, pnl FROM ks_bets
       WHERE ticker = ? AND user_id = ? AND result IN ('win','loss') AND paper = 0`,
      [s.ticker, user.id],
    ).catch(() => [])
    for (const b of settledWrong) {
      const expectedResult = b.side === 'YES' ? (valueFrac === 1 ? 'win' : 'loss')
                                              : (valueFrac === 0 ? 'win' : 'loss')
      if (b.result !== expectedResult) {
        console.warn(`[kalshi-settle] CONTRADICTION bet ${b.id} (${b.side} ${s.ticker}): local=${b.result} but kalshi says ${expectedResult} — fixing`)
        await db.run(
          `UPDATE ks_bets SET result=? WHERE id=?`,
          [expectedResult, b.id],
        ).catch(() => {})
      }
    }
  ...
  for (const s of settlements) {
    ...
    const bets = await db.all(
      `SELECT id, filled_contracts, pnl
       FROM ks_bets WHERE ticker = ? AND user_id = ? AND result IN ('win','loss')`,
      [s.ticker, user.id],
    )
    ...
    for (const bet of bets) {
      // Only overwrite pnl when the system hasn't already set one — the system's real-time
      // value (set the moment a pitcher hits threshold or gets pulled) is what LOCKED shows.
      // Kalshi's actual payout differs due to fee calculation timing; overwriting it causes
      // the LOCKED display to drop unexpectedly hours after bets are determined.
      if (bet.pnl != null) continue
