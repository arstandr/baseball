...
export async function fetchLivePitcherData(date) {
  ...
  for (const g of games) {
    ...
    // Emit delayed-start entries so the dashboard can show the delay reason
    // even before the game goes Live. These are also added to previewGames so
    // we get a boxscore, but we also emit a lightweight record immediately.
    if (status === 'Preview' && /delayed/i.test(detail)) {
      for (const side of ['away', 'home']) {
        const prob = g.teams?.[side]?.probablePitcher
        if (!prob || !pitcherIds.has(String(prob.id))) continue
        const away = g.teams?.away?.team?.abbreviation || 'AWAY'
        ...
        results.push({
          pitcher_id:   String(prob.id),
          pitcher_name: prob.fullName || String(prob.id),
          ks: 0, ip: 0, bf: 0, pitches: 0,
          still_in: null, is_final: false, is_postponed: false,
          is_warmup: false, is_delayed: true,
          delay_reason: detail.replace(/delayed(?: start)?:?\s*/i, '').trim() || 'Unknown',
          game:         `${away}@${home}`,
          game_status:  detail,
          inning:       detail,
          home_score: null, away_score: null, inning_state: null, is_pitching: false,
  ...
  for (let gi = 0; gi < gamesToFetch.length; gi++) {
    ...
    const isWarmup  = detail === 'Warmup'
    const isDelayed = /delayed/i.test(detail)
    const delayReason = isDelayed
      ? (detail.replace(/delayed(?: start)?:?\s*/i, '').trim() || 'Unknown')
      : null
    const inning  = isFinal ? 'Final' : !isLive ? (detail || status) : (isWarmup ? 'Warmup' : (ls?.currentInningOrdinal || detail))
    const away    = g.teams?.away?.team?.abbreviation || 'AWAY'
    const home    = g.teams?.home?.team?.abbreviation || 'HOME'
    ...
    for (const side of ['home', 'away']) {
      ...
      const isPitching = isLive && !isFinal && !isWarmup && !!starter.still_in && (starter.pitches ?? 0) > 0 &&
