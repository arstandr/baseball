...
/** P(K ≥ n) under NB(μ, r). r defaults to NB_R if omitted — backward-compatible. */
export function pAtLeast(mu, n, r = NB_R) {
  return Math.max(0, 1 - nbCDF(mu, r, n - 1))
}

/**
 * Per-archetype NB dispersion r, derived from pitcher_statcast row.
 * Prefers a fitted nb_r from fitDispersion.js if available.
 * Falls back to archetype groups by k_pct:
 *   power  (k_pct ≥ 0.28) → r=20 (tighter, concentrated near mean)
 *   contact(k_pct ≤ 0.19) → r=50 (wider, heavier tails from variance)
 *   mixed                  → r=30 (league baseline)
 */
export function archetypeR(savant) {
  if (!savant) return NB_R
  if (savant.nb_r != null) return savant.nb_r
  const k = savant.k_pct
  if (k == null) return NB_R
  if (k >= 0.28) return 20
  if (k <= 0.19) return 50
  return 30
}
