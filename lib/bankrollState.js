...
export async function initBankrollState(date, morningBankroll = null) {
  // Try to fetch live balance from the first active live bettor
  let bankroll = morningBankroll
  if (bankroll == null) {
    try {
      const liveUser = await db.one(
        `SELECT kalshi_key_id, kalshi_private_key, starting_bankroll FROM users
      ...
      if (liveUser?.kalshi_key_id) {
        ...
        bankroll = bal?.balance_usd ?? liveUser.starting_bankroll ?? Number(process.env.STARTING_BANKROLL || 5000)
      }
    } catch {}
    if (bankroll == null) {
      bankroll = Number(process.env.STARTING_BANKROLL || 5000)
    }
  }
