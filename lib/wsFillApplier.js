...
export async function applyFillEvent(userId, payload) {
  const row = await db.one(
    'SELECT id, filled_contracts, fill_price, order_status FROM ks_bets WHERE order_id = ? AND user_id = ?',
  ...
  const priceCents =
    Math.round(parseFloat(payload.yes_price_dollars ?? payload.no_price_dollars ?? '0') * 100) || null
