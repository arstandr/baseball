...
export async function notifyCrossedYes({ pitcherName, strike, currentKs, yesAskCents, contracts, expectedProfit, game, paper }, webhooks) {
  const mode = paper ? '[PAPER]' : '[LIVE]'
  await send({
    embeds: [{
...
export async function notifyBlowout({ pitcherName, strike, currentKs, scoreDiff, currentInn, contracts, askCents, expectedProfit, game, paper }, webhooks) {
  const mode = paper ? '[PAPER]' : '[LIVE]'
