...
import { TEAM_TO_KALSHI, NBA_TEAM_TO_KALSHI } from './teams.js'

export { TEAM_TO_KALSHI, NBA_TEAM_TO_KALSHI }
...
export async function authedRequest(method, url, body, params, creds = {}, _attempt = 0) {
  ...
  // 429 rate-limit: back off and retry (up to 3 times: 2s, 4s, 8s)
  if (res.status === 429 && _attempt < 3) {
    const delay = (2 ** (_attempt + 1)) * 1000
    console.warn(`[kalshi] 429 rate-limit on ${method} ${url} — retrying in ${delay}ms (attempt ${_attempt + 1}/3)`)
    await new Promise(r => setTimeout(r, delay))
    return authedRequest(method, url, body, params, creds, _attempt + 1)
  }
  if (res.status >= 400) {
 ...
 */
// ─── PAPER-MODE WRAPPER ───────────────────────────────────────────
// When KALSHI_PAPER_MODE=true (or =1), all order-mutating calls are
// short-circuited: no Kalshi HTTP request is made. placeOrder returns a
// synthetic "filled" response so downstream code (ks_bets logging,
// dashboard display, Discord notifications, settlement) behaves
// indistinguishably from a real fill at the requested price.
//
// To turn off: unset KALSHI_PAPER_MODE or set to anything other than
// 'true' / '1'. Single env var flip — no code change needed.
//
// Synthetic order_ids are prefixed `paper-<timestamp>-<rand>` so they
// can be identified later if cleanup is ever needed.
function isKalshiPaperMode() {
  const v = String(process.env.KALSHI_PAPER_MODE ?? '').toLowerCase()
  return v === 'true' || v === '1' || v === 'yes'
}

// Startup banner — fires once at module load so logs prove which mode
// the running process is in, without waiting for an order attempt.
console.log(`[kalshi] startup mode: ${isKalshiPaperMode() ? 'PAPER (synthetic fills)' : 'LIVE (real money)'}  KALSHI_PAPER_MODE=${process.env.KALSHI_PAPER_MODE ?? '(unset)'}`)

function syntheticOrderId() {
  return `paper-${Date.now()}-${Math.floor(Math.random() * 1e6)}`
}

export async function placeOrder(ticker, side, contracts, price, creds = {}, action = 'buy', opts = {}) {
  ...
  if (isKalshiPaperMode()) {
    const orderId = syntheticOrderId()
    const now = new Date().toISOString()
    const synthOrder = {
      order_id:        orderId,
      client_order_id: opts?.client_order_id ?? `mlbie-${Date.now()}-${Math.floor(Math.random() * 1e6)}`,
      ticker, side, action, type: 'limit',
      [side === 'yes' ? 'yes_price' : 'no_price']: priceCents,
      status:          'filled',
      filled_count:    contracts,
      fill_count_fp:   String(contracts),
      remaining_count: 0,
      count:           contracts,
      created_time:    now,
      filled_time:     now,
    ...
    console.log(`  [kalshi:PAPER] ${side.toUpperCase()} ${contracts}c @ ${priceCents}¢ on ${ticker}  →  synthetic fill ${orderId}`)
    return { order: synthOrder }
  }
  // ─────────────────────────────────────────────────────────────

  // Caller may supply a deterministic client_order_id (Gateway uses
  // gateway_<sha256(decision_id):0..16>). Fallback to legacy random for any
  // direct lib/kalshi caller that hasn't migrated.
  const clientOrderId = (opts && typeof opts.client_order_id === 'string' && opts.client_order_id.length > 0)
    ? opts.client_order_id
    : `mlbie-${Date.now()}-${Math.floor(Math.random() * 1e6)}`
...
export async function cancelOrder(orderId, creds = {}) {
  if (!orderId) return null
  // Paper mode: ALL cancels short-circuit (we don't track resting orders
  // in paper mode anyway since synthetic fills are filled immediately).
  if (isKalshiPaperMode()) {
    console.log(`  [kalshi:PAPER] cancelOrder(${orderId}) — no-op`)
    return { cancelled: true, order_id: orderId, _paper_mode: true }
  }
  try {
...
export async function cancelAllOrders({ ticker, status } = {}, creds = {}) {
  // Paper mode: no-op
  if (isKalshiPaperMode()) {
    console.log(`  [kalshi:PAPER] cancelAllOrders(ticker=${ticker ?? '*'}) — no-op`)
    return { cancelled_count: 0, order_ids: [], _paper_mode: true }
  }
  const params = {}
...
/**
 * All open market positions with per-position detail.
 * Returns market_positions[] — each has ticker, position (signed int), market_exposure_dollars.
 * Positive position = YES contracts held; negative = NO contracts held.
 */
export async function getMarketPositions(creds = {}) {
  const data = await authedRequest('GET', '/portfolio/positions', null, { count_filter: 'position', limit: 200 }, creds)
  return data?.market_positions || []
}

/**
 * Single market record by ticker. Returns the market object or null.
 */
export async function getMarket(ticker, creds = {}) {
  try {
    const data = await authedRequest('GET', `/markets/${ticker}`, null, null, creds)
...
export async function getOrderbook(ticker, depth = 10, creds = {}) {
  ...
  try {
    ...
    // Kalshi response shapes seen:
    //   legacy:  { orderbook: { yes: [[price, contracts], ...], no: [...] } }
    //   current: { orderbook_fp: { yes_dollars: [[price, dollars], ...], no_dollars: [...] } }
    // In the current shape, qty is dollars-at-this-price. Convert to contracts (dollars / price).
    const book = data?.orderbook_fp ?? data?.orderbook ?? data
    if (!book) return null
    const isFp = !!data?.orderbook_fp
    ...
    const normalizeLevel = ([price, qty]) => {
      const p = parseCents(price) ?? 0
      let q = Number(qty) || 0
      if (isFp && p > 0) q = Math.round((q * 100) / p)  // dollars → contracts
      return [p, q]
    }
    const yes = ((isFp ? book.yes_dollars : book.yes) || []).map(normalizeLevel).filter(([p]) => p > 0)
    const no  = ((isFp ? book.no_dollars  : book.no)  || []).map(normalizeLevel).filter(([p]) => p > 0)
...
// ===========================================================================
// NBA Totals helpers — implemented in lib/kalshiNBA.js, re-exported here
// for backward compatibility.
// ===========================================================================
export { buildNBATotalEventTicker, findBestNBATotalMarket, getNBATotalMarkets, listNBAGamesFromKalshi } from './kalshiNBA.js'
