...
// ------------------------------------------------------------------
function _buildAuthHeaders(method, path, { json = false } = {}) {
  const keyId   = process.env.KALSHI_KEY_ID
  const keyPath = process.env.KALSHI_KEY_PATH
  if (!keyId || !keyPath) throw new Error('kalshi: missing KALSHI_KEY_ID or KALSHI_KEY_PATH')
...
async function authedRequest(method, url, body, params) {
  // Kalshi signs only the path (no query string) per their API docs
  const sigPath = `/trade-api/v2${url}`
  const headers = _buildAuthHeaders(method, sigPath, { json: !!body })
 ...
 */
export function getAuthHeaders(method, path, { json = false } = {}) {
  return _buildAuthHeaders(method, path, { json })
}
...
export async function getBalance() {
  const data = await authedRequest('GET', '/portfolio/balance')
  return {
 ...
 */
export async function placeOrder(ticker, side, contracts, price) {
  ...
  const data = await authedRequest('POST', '/portfolio/orders', body)
  return data
}
